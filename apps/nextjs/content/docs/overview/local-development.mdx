...
Learn how to effectively develop with Kit, from daily workflows to debugging techniques. This guide covers everything you need to know to be productive with the Kit boilerplate.
...
Kit uses Turborepo to manage multiple packages efficiently:
...
Kit maximizes code sharing between web and mobile:
...
Kit uses Drizzle ORM for type-safe database operations:
...
Kit uses tRPC for type-safe APIs. Add routes in `packages/api/src/`:
...
Kit provides a shared UI package:
...
Kit enforces code quality automatically:
