...
- **`packages/api/src/auth/auth.ts`** - the `oAuthProxy` fallback `productionURL` is `kit.bigdropincs.com`; on Vercel it's derived from `VERCEL_PROJECT_PRODUCTION_URL`, but update the fallback anyway
