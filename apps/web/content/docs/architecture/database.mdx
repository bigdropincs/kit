...
Kit uses Supabase (Postgres) with Drizzle ORM for type-safe queries. This guide covers the setup, schema, and development workflow.
...
Kit uses `drizzle-kit push` - schema goes straight to the database, no migration files.
